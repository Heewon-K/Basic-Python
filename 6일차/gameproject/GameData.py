...
class GameData:  # 클래스명은 가급적 첫글자는 대문자로 하자 
    COMWIN=1
    PERWIN=2
    DRAW=3 
    ...
    def whoIsWinner(self):  # gameinfo 라는 dict타입대신에 self로 
    ...
    def output(self):
        words=["", "가위", "바위", "보"]
        words2=["", '컴퓨터승', '사람승', '무승부']
        print(f"컴퓨터:{words[self.computer]} 사람:{words[self.person]} 승부:{words2[self.winner]}")
